        ...
        <div v-show="!spinner" class="card-deck d-flex flex-wrap justify-content-center my-3">
            <div v-for="user in listOfUsers" :key="user.id" class="card my-2" style="width: 9rem; min-width: 9rem; flex: 0;">
                ...
                <div class="card-footer d-flex justify-content-around p-2">
                    <a type="button" title="Description" data-toggle="modal" :data-target="`#descriptionModal${user.id}`" style="color: teal;">
                    ...
                    <a type="button" :href="`contactUsers/${user.id}`" data-toggle="tooltip" data-placement="top" title="Envoyer un message" style="color: teal;">
                    ...
                    <a @click="deleteContact(user.id)" type="button" data-toggle="tooltip" data-placement="top" title="Supprimer ce contact" style="color: red;">
                ...
                <!-- Modal Description-->
                <div class="modal fade" :id="`descriptionModal${user.id}`" tabindex="-1" aria-labelledby="descriptionModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="descriptionModalLabel">{{ user.name }}</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <div class="modal-body">
                                <p class="font-weight-bold">{{ user.job }}</p>
