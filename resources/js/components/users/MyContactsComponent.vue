...
<div>
    <div>
        <a :href="`coweerkers`" class="btn btn-perso my-2 position-relative" style="left: 50%; transform: translateX(-50%);">Voir la liste des coweerkers</a>
        ...
        <div class="card-deck d-flex flex-wrap justify-content-center my-3">
            <div v-for="myContact in myContacts" :key="myContact.id" class="card" style="width: 9rem; min-width: 9rem; flex: 0;">
                <img :src="`/storage/${myContact.avatar}`" class="card-img-top rounded-circle m-auto pt-2" style="width: 7rem;" :alt="`Photo de profil de ${myContact.name}`">
                ...
                <div class="card-footer d-flex justify-content-around p-2">
                    <a title="Description" data-toggle="modal" :data-target="`#descriptionModal${myContact.id}`" style="color: teal; cursor: pointer;">
                    ...
                    <a :href="`contactUsers/${myContact.id}`" data-toggle="tooltip" data-placement="top" title="Envoyer un message" style="color: teal; cursor: pointer;">
                    ...
                    <a @click="removeContact(myContact.id)" data-toggle="tooltip" data-placement="top" title="Supprimer des favoris" style="color: red; cursor: pointer;">
...
export default {
    ...
    methods: {
        ...
        getContacts(){
            ...
            axios.get('https://lecowee.astrid-perillat.fr/myContacts')
            .then((response) => {
